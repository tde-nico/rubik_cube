...
class Game(Ursina):
	...
	def load_game(self):
		...
		self.cube_side_positions = {'LEFT': self.LEFT, 'BOTTOM': self.BOTTOM, 'RIGHT': self.RIGHT,
	...
	def random_state(self):
		[self.rotate_side_without_animation(choice(list(self.rotation_axes))) for i in range(ROTATIONS)]

	def rotate_side_without_animation(self, side_name):
		cube_positions = self.cube_side_positions[side_name]
	...
	def create_sensors(self):
		create_sensor = lambda name, pos, scale: Entity(name=name, position=pos, model='cube',
			color=color.dark_gray, scale=scale, collider='box', visible=False)
	...
	def toggle_game_mode(self):
		...
		msg = dedent(f"{'ACTION' if self.action_mode else 'VIEW'} mode ON (to switch - press P)").strip()
	...
	def input(self, key):
		if key in 'zx' and self.action_mode and self.action_trigger:
			for hitinfo in mouse.collisions:
				collider_name = hitinfo.entity.name
				if (key == 'z' and collider_name in 'LEFT RIGHT FACE BACK' or
					key == 'x' and collider_name in 'TOP BOTTOM'):
